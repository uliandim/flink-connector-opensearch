...
import org.apache.flink.connector.opensearch.sink.Opensearch3Sink;
import org.apache.flink.connector.opensearch.sink.Opensearch3SinkBuilder;
...
/**
 * A {@link DynamicTableSink} that describes how to create a {@link Opensearch3Sink} from a logical
 * description.
 */
@Internal
class Opensearch3DynamicSink implements DynamicTableSink {
    ...
    public SinkRuntimeProvider getSinkRuntimeProvider(Context context) {
        ...
        final RowOpensearch3Emitter rowOpensearchEmitter =
                new RowOpensearch3Emitter(
                        createIndexGenerator(), format, createKeyExtractor());

        final Opensearch3SinkBuilder<RowData> builder = new Opensearch3SinkBuilder<>();
        builder.setEmitter(rowOpensearchEmitter);
        builder.setHosts(
                config.getHosts().stream()
                        .map(host -> new HttpHost(host.getSchemeName(), host.getHostName(), host.getPort()))
    ...
    public DynamicTableSink copy() {
        return new Opensearch3DynamicSink(
                format,
                config,
    ...
    public boolean equals(Object o) {
        ...
        Opensearch3DynamicSink that = (Opensearch3DynamicSink) o;
